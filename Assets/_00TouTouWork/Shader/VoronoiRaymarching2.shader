...
Shader "TouTou/VoronoiRaymarching2"
{
	SubShader
	{
		...
		Pass
		{
			...
			float4 voronoi(float3 p)
			{
				...
				//return float4(mp1+ist,smoothstep(0,0.05,min_dm12));
				return float4(mp1+ist,step(0.01,min_dm12));
			}

			float4 dfs(float3 p)
			{
				float4 v = voronoi(p);
				float d1 = sphere(p,v.xyz,0.015);
				//float d2 = grid(p);
				float d2 = v.w;
			...
			float3 getNormal(float3 p)
			{
				...
				return normalize(float3(
					dfs(p + float3(d, 0, 0)).w - dfs(p - float3(-d, 0, 0)).w,
					dfs(p + float3(0, d, 0)).w - dfs(p - float3(0, -d, 0)).w,
					dfs(p + float3(0, 0, d)).w - dfs(p - float3(0, 0, -d)).w
				));
			}
			...
			float4 frag(v2f i):COLOR
			{
				...
				int colitr=0;
				for (int i = 0; i < 7; ++i) { //レイマーチングのループを実行
					p = ro + rd * t;
					float4 vd = dfs(p);
					v = vd.xyz;
					d = vd.w;
				...
				if (d > thred) { //レイが衝突していないと判断すれば黒に描画する
					...
					float3 baseCol = float3(0.7,0.7,0.7)*fog;
					col =  float4(baseCol,1);
					//col = float4(baseCol,1)+float4(baseCol*NdotL*9,1);
				}
				return col;
			}
			ENDCG
